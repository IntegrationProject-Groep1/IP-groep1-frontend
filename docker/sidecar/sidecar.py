...
RABBITMQ_PORT = int(os.environ.get("RABBITMQ_PORT", "30000"))
...
try:
    ...
    for target in TARGETS_RAW.split(","):
        host, port_str = target.strip().rsplit(":", 1)
...
def connect_rabbitmq():
    ...
    while True:
        try:
            ...
            print(f"Verbonden met RabbitMQ op {RABBITMQ_HOST}:{RABBITMQ_PORT} ({RABBITMQ_VHOST})")
            return connection, channel
        except pika.exceptions.AMQPConnectionError as exc:
            print(f"RabbitMQ verbinding mislukt: {exc}. Opnieuw proberen in 5 sec")
            time.sleep(5)
    ...
    if all_alive(TARGETS):
        ...
        try:
            ...
            try:
                ...
                print(f"Fout bij sluiten van RabbitMQ verbinding: {exc}")
            connection, channel = connect_rabbitmq()
    else:
        uptime_seconds = 0
